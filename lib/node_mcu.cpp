      ...
      if (client.available()) { // if there's bytes to read from the client,
        ...
        if (c == '\n') { // if the byte is a newline character
          ...
          if (currentLine.length() == 0) {
            ...
            client.println(".buttonRed {  background-color: grey; border: none; color: white; padding: 16px 40px; border-radius: 60%;");
            ...
            // Web Page Heading
            client.println("<body><h1>SMART LIGHT</h1>");

            // Display current state, and ON/OFF buttons for GPIO 2 Red LED
            client.println("<p>LED is " + outputState + "</p>");
            // If the outputState is off, it displays the OFF button
            if (outputState == "off") {
